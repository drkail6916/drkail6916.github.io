...
- 활용 예시
  - 데이터 확인: 특정 값 존재 여부.
    ...
    ```
- 주의점
  - 대량 데이터에서는 `set`으로 변환 후 `in` 사용(속도 개선).
  - pandas에서는 `isin()` 선호.

#### 24. `is`
- 기능
  - 객체의 동일성 비교(메모리 주소 동일 여부).
- 사용법
  - `obj1 is obj2`.
- 예시
  ```python
  x = None
  if x is None:
      print("None detected")  # 출력: None detected
  ```
- 활용 예시
  - 결측값 확인: `is None` 또는 `pd.isna()`.
    ```python
    if df['value'].iloc[0] is None:
        print("Missing value")
    ```
- 주의점
  - `is`는 `==`와 다름(값 비교 아님).
  - 소규모 정수는 캐싱으로 `is`가 예상치 못한 결과 반환 가능.
...
#### 26. `nonlocal`
- 기능
  - 상위 스코프의 변수를 참조/수정(전역 아님).
- 사용법
  - `nonlocal var`로 상위 스코프 변수 선언.
- 예시
  ```python
  def outer():
      x = 0
...
- 활용 예시
  - 드물게 사용. 중첩 함수로 상태 관리 시 유용.
    ...
    ```
- 주의점
  - `nonlocal`은 전역 변수가 아님. 스코프 혼동 주의.

#### 27. `not`
- 기능
  - 논리 부정 연산자.
- 사용법
  - `not condition`: 조건을 반전.
- 예시
  ```python
  x = False
  if not x:
      print("Not False")  # 출력: Not False
  ```
- 활용 예시
  - 조건 반전: 데이터 필터링.
    ```python
    mask = ~df['value'].isna()  # pandas에서는 ~을 사용.
    ```
- 주의점
  - pandas에서는 `~`로 벡터화 부정.
  - 단락 평가로 인한 로직 주의.
...
#### 28. `or`
- 기능
  - 논리 합 연산자. 하나라도 `True`면 `True` 반환.
  - 단락 평가: 첫 번째 조건이 `True`면 두 번째 조건 평가 안 함.
- 사용법
  - `A or B`.
- 예시
  ```python
  x, y = 0, 10
  if x == 0 or y > 5:
      print("At least one true")  # 출력
  ```
- 활용 예시
  - 복합 조건: 데이터 필터링.
    ```python
    filtered = df[ (df['A']<0) | (df['B']>100) ]  # pandas에서는 |
    ```
- 주의점
  - pandas에서는 `|` 사용.
  - 단락 평가로 두 번째 조건 무시 가능.
...
- 활용 예시
  - 임시 코드: 모델 파이프라인의 플레이스홀더.
    ...
    ```
- 주의점
  - `pass`는 코드 구조 유지에 유용하지만, 실제 구현 대체 불가.
...
- 활용 예시
  - 데이터 검증: 잘못된 입력 방지.
    ```python
    if (df['age'] < 0).any():
        raise ValueError("Negative age detected")
    ```
- 주의점
  - 특정 예외 사용 권장.
  - 예외 메세지는 명확하게.
...
- 예시
  ...
  try:
      ...
      print("Error")  # 출력: Error
  ```
- 활용 예시
  - 데이터 처리 안정성: 결측값, 파일 오류 처리.
    ```python
    try:
        df = pd.read_csv('data.csv')
...
- 예시
  ...
  ```
- 활용 예시
  - 데이터 처리 루프: 조건 기반 데이터 처리.
    ```python
    while len(df) > 1000:
        df = df.sample(frac=0.5)  # 데이터 축소
    ```
- 주의점
  - 무한 루프 방지(조건 명확하게).
  - 대량의 데이터에서는 `for` 또는 벡터화 선호.

#### 34. `with`
- 기능
  - 컨텍스트 관리자. 리소스(파일, 연결 등)를 자동 관리.
- 사용법
...
- 활용 예시
  - 파일 처리: 대량의 CSV 읽기/쓰기.
    ...
    ```
- 주의점
  - 컨텍스트 관리자를 지원하는 객체만 사용 가능.
  - 리소스 누수 방지.

#### 35. `yield`
- 기능
  - 제너레이터 함수에서 값을 하나씩 반환.
  - 메모리 효율적 반복.
- 사용법
  - `yield value`로 값을 반환, 함수 상태 유지.
- 예시
  ```python
  def numbers():
      for i in range(3):
  ...
  ```
- 활용 예시
  - 대량의 데이터 처리: 메모리 절약하며 데이터 스트리밍.
    ```python
    def read_chunks(file):
        for chunk in pd.read_csv(file, chunksize=1000):
            yield chunk
    
    for chunk in read_chunks('large.csv'):
        print(chunk.shape)
    ```
- 주의점
  - 제너레이터는 한 번만 순회 가능.
...
#### 정리
- 핵심 예약어
  - 자주 사용: `import`, `def`, `if`, `for`, `with`, `try`, `except`, `return`, `lambda`.
  - 성능 관련: `for`, `while`은 벡터화(`numpy`, `pandas`)로 대체 권장.
  - 데이터 처리: `None`, `True`, `False`, `and`, `or`는 필터링과 조건 처리에 필수.

- 주의점 요약
  - 예약어는 변수명으로 사용 불가.
  - 대량의 데이터 처리 시 `apply`, `for`, `while` 대신 벡터화 연산(`numpy`, `pandas`) 선호.
  - 디버깅 시 `assert`, `try-except` 활용.
  - 코드 가독성을 위해 명확한 이름과 주석 사용.